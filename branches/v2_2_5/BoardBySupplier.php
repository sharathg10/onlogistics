...
$SupplierArray = SearchTools::createArrayIDFromCollection(
        array('Supplier', 'AeroSupplier'), array(),
		_('Select one or more suppliers'));
$form->addElement('select', 'Id', _('Supplier'),
        array($SupplierArray, 'multiple size="5"'), array());
...
if (true === $form->displayGrid()) {
	...
	if (SearchTools::requestOrSessionExist('NoNullCommands')) {
		$SupplierWithCommandsArray = getSuppliersWithCommands(
                $userConnectedActorId, $MySQLStartDate, $MySQLEndDate, $commandType, $currency);
	    $FilterComponentArray[] = SearchTools::NewFilterComponent(
                'Id', '', 'In', $SupplierWithCommandsArray, 1);
	}
	...
	define('SUPPLIER_LIST_ITEM_PER_PAGE', 100);
	$grid = new Grid();
	$grid->itemPerPage = SUPPLIER_LIST_ITEM_PER_PAGE;
	$grid->withNoCheckBox = true;
	$grid->withNoSortableColumn = true;

	$grid->NewAction('Print', array());
	$grid->NewAction('Export', array('FileName' => 'CAparfournisseur'));

	$grid->NewColumn('FieldMapper', _('Name'), array('Macro' => '%Name%'));
	// Nombre de commandes
	$grid->NewColumn('BoardSupplierCommand', _('Number of orders'),
					 array('req' => 'command_num',
						   'start' => $MySQLStartDate, 'end' => $MySQLEndDate,
						   'commandType' => $commandType, 'currency'=>$currency));
	// ca par supplier
	$grid->NewColumn('BoardSupplierCommand', _('Turnover excl. VAT'),
					 array('req' => 'ca_par_sup',
						   'start' => $MySQLStartDate, 'end' => $MySQLEndDate,
						   'commandType' => $commandType, 'currency'=>$currency));
