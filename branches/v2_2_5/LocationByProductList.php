...
if (empty($_REQUEST['StoreId'])) {
    Template::errorDialog(sprintf(E_MSG_MUST_SELECT_A, _('store')), $returnUrl);
    exit;
}
// pour la gestion des droits
$Store = Object::load('Store', $_REQUEST['StoreId']);
$pageTitle = sprintf(_('Stock state: reference > location, store (%s)'),
                $Store->getName());
...
if (true === $form->displayGrid(1)) {
    ...
    $SQLRequest = request_LocationByProductList($_REQUEST['StoreId']);
    ...
    while (!$rs->EOF){
    	$pq = 'Product'.$i;
    	$$pq = new Product();
    	$$pq->setId($rs->fields['lpqProduct']);
    	$$pq->setBaseReference($rs->fields['pdtBaseReference']);
    	$$pq->setCategory($_REQUEST['StoreId']);
    ...
    $grid->NewAction('Export', array('FileName'=>'StockParReference'));
    ...
    $grid->NewColumn(
        'LocationByProductList',
        '<table style="width: 100%;"><thead><tr>' .
            '<td style="width: 50%">'._('Location').'</td>' .
            '<td style="width: 50%">'._('Quantity').'</td>' .
            '</tr></thead></table>',
