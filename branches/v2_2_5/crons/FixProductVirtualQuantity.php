...
for ($i=0; $i<$count; $i++) {
    ...
    if ($oldVQ != $newVQ) {
        $pdt->setSellUnitVirtualQuantity($newVQ);
        $pdt->save();
...
if (!empty($mailBody)) {
    ...
    $subj  = "Quantités virtuelles corrigées sur la base \"$db\"";
    $body  = "<strong>Détail des corrections des quantités virtuelles ";
    $body .= "sur la base \"$db\":</strong>\n";
    $body .= "<p><table border=\"1\" cellpadding=\"5\">\n";
    $body .= "<tr><th>ID produit</th><th>Réf. produit</th>";
    $body .= "<th>Ancienne QV</th><th>Nouvelle QV</th>\n";
