...
define('I_NOT_DELETED_ENTITY', _('The following theme could not be deleted because it is associated to a question model: %s'));
define('I_NOT_DELETED_ENTITIES',  _('The following themes could not be deleted because they are associated to a question model: %s'));

/**
 *
 */
class ThemeAddEdit extends GenericAddEdit {
    private $_notDeletedEntity = array();

    // ThemeAddEdit::__construct() {{{
    ...
    protected function onBeforeDelete() {
        $objectMapper = Mapper::singleton('Theme');
        $objectCol = $objectMapper->loadCollection(
			array('Id' => $this->objID));

        //pour la vérification dans la boucle
        $questionMapper = Mapper::singleton('Question');
        ...
        for($i=0 ; $i<$count ; $i++){
	        ...
	        /*
	        Ne sont supprimable que les thème liés à 
	        aucune question modèle
	        */
            $questionCol = $questionMapper->loadCollection(
                array('Theme'=>$object->getId()));
            if($questionCol->getCount()==0 ) {
                //on peut supprimer
