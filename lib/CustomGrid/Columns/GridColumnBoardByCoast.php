...
class GridColumnBoardByCoast extends AbstractGridColumn {
     ...
     * @access protected 
     */
    function GridColumnBoardByCoast($title = '', $params = array()) {
        parent::__construct($title, $params);
        if(isset($params['totalCoast'])) {
			$this->totalCost = $params['totalCoast'];
		}
        $this->req = $params['req'];
    ...
    function Render($object) {
        ...
        if (in_array($this->commandType, $takeSupplier)) {
            ...
            $FilterComponentArray[] = SearchTools::NewFilterComponent('Destinator', '', 'Equals', $Auth->getActorId(), 1);
        }
		$FilterComponentArray[] = SearchTools::NewFilterComponent('IsEstimate', "", 'Equals', 0, 1);
		$FilterComponentArray[] = SearchTools::NewFilterComponent('Expeditor', '', 'Equals', $object->getId(), 1);
		$FilterComponentArray[] = SearchTools::NewFilterComponent('CommandSupplier', 'Expeditor.ClassName', 'Equals', 'supplier', 1);
		...
		if($this->req == 'command_num') {
			...
			if($this->req == 'coast_par_fournisseur') {
        		return I18N::formatNumber($ttForThisClient);
        	} else if($this->req == 'coast_percent' && isset($this->totalCost)) {
        		if($this->totalCost == 0) { // NO DIIV BY ZERO
        			return _('N/A');
        		}
        		$percent = $ttForThisClient / $this->totalCost * 100 ;
