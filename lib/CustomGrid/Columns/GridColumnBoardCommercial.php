...
class GridColumnBoardCommercial extends AbstractGridColumn {
     ...
     * @access protected 
     */
    function GridColumnBoardCommercial($title = '', $params = array()) {
    ...
    function Render($object) {
    	...
    	// POUR FILTRER AVEC DATE NE PAS UTILISER FILTR ARRAY MAIS FILTERCOMPONENT POUT CHAQUE ELEMS
		...
		$FilterComponentArray[] = SearchTools::NewFilterComponent('Commercial', "", 'Equals', $object->getId(), 1);
        ...
        if($this->req == 'command_num')
        	...
        	$ttForThisCommercial = 0;
        	for ($i=0; $i<$count; $i++) {
        	 	$item = $commandCollection->getItem($i);
        	 	$ttForThisCommercial += $item->getTotalPriceHT();
        	}
			if($this->req == 'ca_par_com') {
        		return I18N::formatNumber($ttForThisCommercial);
        	} else if($this->req == 'ca_percent' && isset($this->totalCa)) {
        		if($this->totalCa == 0) { // NO DIIV BY ZERO
        			return _('N/A');
        		}
        		$percent = $ttForThisCommercial / $this->totalCa * 100;
