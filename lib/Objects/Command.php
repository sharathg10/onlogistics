...
class Command extends _Command {
    ...
    */
    ...
    function getDataForRTWBL($DateSup='') {
        ...
        foreach ($data as $item) {
            ...
            if (!($rtwProduct instanceof RTWProduct)) { 
                $rtwProduct = Object::load('Product', array('BaseReference' => $item[0]));
                if (!($rtwProduct instanceof Product)) { 
                    continue;
                }
            }
            ...
            if (!isset($ret[$modelId])) {
                ...
                $ret[$modelId][1] .= ($size ? ", ".$size->getName().": {$item[2]}" : '');
                $ret[$modelId][2] += intval($item[2]);
                $ret[$modelId][4] += intval($item[4]);
                $ret[$modelId][5] += intval($item[5]);
            }
        }
     ...
     * @return void
     */
    function BlockDeblock($block=1, $useTransaction=true) {
        require_once('Objects/Command.const.php');
        require_once('Objects/ActivatedMovement.php');
