...
foreach($GLOBALS['DSNS'] as $dsn_name) {
    ...
    $flowTypeCol = Object::loadCollection('FlowType');
    foreach($flowTypeCol as $flowType) {
        $flowTypeItemCol = $flowType->getFlowTypeItemCollection();
        if($flowTypeItemCol->getCount()>0) {
            continue;
        }
        $flowTypeItem = new FlowTypeItem();
        $flowTypeItem->Name = 'Montant';
        $flowTypeItem->setFlowType($flowType);
        $flowTypeItem->save();
    }
}
