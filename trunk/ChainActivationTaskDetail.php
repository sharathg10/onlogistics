...
if ($isActivationTask) {
    $ChainArrayId = SearchTools::createArrayIDFromCollection(
            'Chain', array(), '', 'Reference');
    // Il reste a concatener avec l'attribut Description
    foreach($ChainArrayId as $id => $value) {
    	$Chain = Object::load('Chain', $id);
        /*
    	$tab = $Chain->LoadTaskName();
    	if ($Chain->hasTypeTask(TASK_ACTIVATION, $tab)) {
    	    unset($ChainArrayId[$id]);
    		continue;
    	}
        */
    	$ChainArrayId[$id] .= ' ' .  $Chain->getDescription();
    }
    $form->addElement('select', 'ChainToActivate', _('Chain to activate'),
        $ChainArrayId, 'onchange="toggleProductCommandTypeDiv(this.value)" ');
    ...
    // si ce n'est pas une tâche d'activation, ca peut être la tâche pivot.
    $pivotDateArray = array(-1=>A_NO) + ActivatedChain::getPivotDateTypeConstArray();
    $pivotDateArray[1] = _('task beginning date/time');
    $pivotDateArray[2] = _('task end date/time');
    $form->addElement('select', 'PivotDate', _('Deadline date'), $pivotDateArray,
        'onchange="modifyPivotDate(this.value);"');
}

$NomArrayId = getNomenclatureArray();
$form->addElement('select', 'Nomenclature', _('Nomenclatures'), $NomArrayId,
				  'onchange="updateComponents(this.value)" '
				  . 'id="Nomenclature" style="width: 100%"');
$form->addElement('select', 'Component', _('Components'), array(),
                  'multiple="multiple" size="6" id="Component" style="width: 100%"');
...
$Smarty->assign('isActivationTask', $isActivationTask);
