/**
 * $Source: /home/cvs/onlogistics/js/includes/PrestationAddEdit.js,v $
 *
 * Ajoute deux grids éditables, pour Prestation.CostRange et
 * Prestation.PrestationCustomer.
 *
 * @todo rendre le tout dynamique
 * @version   CVS: $Id: PrestationAddEdit.js,v 1.10 2008-05-07 16:36:39 ben Exp $
...
entities.push('PrestationCustomer');
entitiesFields['PrestationCustomer'] = ['Actor', 'Name', 'Prestation'];

var server_data = new Object();
var server_collections = ['CostRange', 'PrestationCustomer', 'CostType', 'Zone', 'Store', 'ProductType', 'Actor', 'SellUnitType'];
/**
...
initialize = function() {
    ...
    d[2] = fw.ajax.call('callStaticMethod', 'CostRange','getCostTypeConstArray');
    d[3] = fw.ajax.call('loadCollection', 'Zone');
    d[4] = fw.ajax.call('loadCollection', 'Store', {'Activated':1},
        {'Name':'SORT_ASC'});
    d[5] = fw.ajax.call('loadCollection', 'ProductType');
    d[6] = fw.ajax.call('loadCollection', 'Actor', 
        {'Active':1, 'Generic':0}, {'Name':'SORT_ASC'});
    d[7] = fw.ajax.call('loadCollection', 'SellUnitType');
...
newItem = function(entity, itemData) {
    ...
    if (typeof(itemData) != 'undefined') {
        ...
        // todo dynamic!
        if(entity=='CostRange') {
            var cost = fw.i18n.formatNumber(itemData.cost, 3);
            var costType = itemData.costType.value;
            var beginRange = fw.i18n.formatNumber(itemData.beginRange);
            var endRange = fw.i18n.formatNumber(itemData.endRange);
    ...
    if(entity=='CostRange') {
        appendChildNodes(row, TD(
            ...
            TD(
                null,
                newSelect(
                    'CostType', index, 'CostRange_CostType[]', costType)
            ),
            TD(
                null,
                INPUT({
            ...
            TD(
                null,
                newSelect(
                    'Zone', index, 'CostRange_DepartureZone[]', departureZone, true)
            ),
            TD(
                null,
                newSelect(
                    'Zone', index, 'CostRange_ArrivalZone[]', arrivalZone, true)
            ),
            TD(
                null,
                newSelect(
                    'Store', index, 'CostRange_Store[]', store, true)
            ),
            TD(
                null,
                newSelect(
                    'ProductType', index, 'CostRange_ProductType[]', productType, true)
            ),
            TD(
                null,
                newSelect(
                    'SellUnitType', index, 'CostRange_UnitType[]', unitType, true)
            )
        );
    }
    if(entity=='PrestationCustomer') {
        ...
        appendChildNodes(row, TD(
                null,
                newSelect(
                    'Actor', index, 'PrestationCustomer_Actor[]', actor)
            ),
            TD(
                null,
                INPUT({
...
displayLayers = function() {
    var type = $('Prestation_Type').value;
    
    fw.dom.toggleElement('stockageLayer', 'false');
    fw.dom.toggleElement('maintenanceLayer', 'false');
    
    if(type==0) {
        fw.dom.toggleElement('maintenanceLayer', true);
    } else if(type==3) {
        fw.dom.toggleElement('stockageLayer', true);
    }
}
