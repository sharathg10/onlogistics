...
function handleCommand($commandType, $pdtCollection, $curID, $isEstimate=false, $fromEstimateId=0) {
    ...
    $commandData = array(
            ...
            'Port'            => $_REQUEST['Port'],
            'Packing'         => $_REQUEST['Emballage'],
            'Insurance'       => $_REQUEST['Assurance'],
            'Installment'     => $_REQUEST['Installment'],
            'Handing'         => $_REQUEST['GlobalHanding'],
...
/**
 * Construit et envoie le mail de confirmation de commande
 * @access public
 * @param $ProductCommand Object
 * @param $ProductCommandItemCollection Object Collection
 * @param $Auth Object
 * @param $commandType integer: 1 pour commande client et 2 pour commande fournisseur
 * @param $Customer Object (et ='' si commande Fournisseur)
 * @return void
 */
function sendProductCommandMail($ProductCommand, $commandType, $Customer='') {
    require_once('Objects/Command.const.php');
    require_once('Objects/TVA.inc.php');
    require_once('CalculatePriceHanding.php');
    require_once('GetAddressItems.php');
    // Création du mail avec smarty
    $mailSmarty = new Template();
    $ProductCommandItemCollection = $ProductCommand->getCommandItemCollection();
    // Expediteur
    $Exp = $ProductCommand->getExpeditor();
    $expSite = $ProductCommand->getExpeditorSite();
    $NewsDeliveryExp = Tools::getValueFromMacro($Exp, '%MainSite.Zone.Name%');

    $Expeditor = array_merge(getAddressItems($expSite,
            $expSite->getPhone()), array('Name' => $Exp->getName()));

    // Destinataire
    $Dest = $ProductCommand->getDestinator();
    $destSite = $ProductCommand->getDestinatorSite();
    $NewsDeliveryDest = Tools::getValueFromMacro($destSite, '%Zone.Name%');

    $Destinator = array_merge(getAddressItems($destSite,
            $destSite->getPhone()), array('Name' => $Dest->getName()));
    $Commercial = $ProductCommand->getCommercial();

    // Affectation des variables smarty
    // devise
    $cur = $ProductCommand->getCurrency();
    $currency = $cur instanceof Currency?$cur->getSymbol():'&euro;';
    $mailSmarty->assign('currency', $currency);

    $sc = $ProductCommand->getSupplierCustomer();
    $hasTVA = $sc->getHasTVA();

    $incoterm = $ProductCommand->getIncoterm();
    if ($incoterm instanceof Incoterm) {
        $mailSmarty->assign('incoterm', $incoterm->toString());
    }

    $mailSmarty->assign('reference', $ProductCommand->getCommandNo());
    $dateReception = I18N::formatDate($ProductCommand->getCommandDate());
    $mailSmarty->assign('dateReception', $dateReception);
    $mailSmarty->assign('expeditor', $Expeditor);
    $mailSmarty->assign('destinator', $Destinator);
    $mailSmarty->assign('cadencedOrder', $ProductCommand->getCadenced());
    if ($Commercial instanceof UserAccount) {
        $mailSmarty->assign('Commercial', $Commercial->getIdentity());
    }

    $productArray = array();
    $GlobalHanding = $ProductCommand->getHanding();  // remise globale
    $handingByRange = $ProductCommand->getHandingByRangePercent(); // remise par tranche
    if ($handingByRange > 0) {
        $mailSmarty->assign('HandingByRangePercent', sprintf(I_COMMAND_HANDING, $handingByRange));
    }
    $supplier = $ProductCommand->getExpeditor();
    for($i = 0; $i < $ProductCommandItemCollection->getCount(); $i++) {
        unset($ProductCmdItem, $product);
        $ProductCmdItem = $ProductCommandItemCollection->getItem($i);
        $product = $ProductCmdItem->getProduct();
        $numberUBInUV = $product->getNumberUBInUV();
        $numberUB = $numberUBInUV * $ProductCmdItem->getQuantity();
        // On affiche, si elle existe, la ref fournisseur ou client, selon
        $actorForRef = ($commandType==Command::TYPE_CUSTOMER)?
                $Dest:$supplier;
        $productRef = $product->getReferenceByActor($actorForRef);
        $productRef = ($productRef != '')?$productRef:$product->getBaseReference();

        $unitQty = $commandType==Command::TYPE_CUSTOMER?
            $product->getSellUnitQuantity():$product->getBuyUnitQuantity($supplier);
        $unitQty = $unitQty?$unitQty:$product->getSellUnitQuantity();
        $tva = $ProductCmdItem->getTVA();
        $TVARate = (!Tools::isEmptyObject($tva))?I18N::formatNumber($tva->getRate()):'';
        $promotion = $ProductCmdItem->getPromotion();

        if ($promotion instanceof Promotion) {
            $promoRate = I18N::formatNumber($promotion->getRate());
        	if($promotion->getType()==Promotion::PROMO_TYPE_PERCENT) {
        	    $promoRate = I18N::formatPercent($promotion->getRate());
        	} elseif ($promotion->getType()==Promotion::PROMO_TYPE_MONTANT) {
        	    $currency = $promotion->getCurrency();
        	    $promoRate = ($currency instanceof Currency)?
        	       I18N::formatCurrency($currency->getSymbol(), $promotion->getRate()):'';
        	}
        } else {
            $promoRate = '';
        }
        $productArray[] = array (
            ...
            'Handing' => $ProductCmdItem->getDisplayedHanding(),
            'unitPrice' => I18N::formatNumber($ProductCmdItem->getPriceHT()),
            'PromotionRate' => $promoRate,
            'TVARate' => $TVARate,
            'htTotal' => $ProductCmdItem->getTotalHT(true),
            'ttcTotal' => $ProductCmdItem->getTotalTTC(true),
            'numberUB' => $commandType==Command::TYPE_CUSTOMER?$numberUB:'N/A',
            'numberUE' => $ProductCmdItem->getPackagingUnitQuantity(),
    ...
    /*$fraisHT  = $ProductCommand->getPort() + $ProductCommand->getPacking()
                + $ProductCommand->getInsurance();

    if ($hasTVA) {
        $fraisTTC = $fraisHT
            + ($ProductCommand->getPort() * getTVARateByCategory(TVA::TYPE_DELIVERY_EXPENSES) / 100)
            + ($ProductCommand->getPacking() * getTVARateByCategory('TVA::TYPE_PACKING) / 100)
            + ($ProductCommand->getInsurance() * getTVARateByCategory(TVA::TYPE_INSURANCE) / 100);
    } else {
        $fraisTTC = $fraisHT;
    }*/

    if ($commandType == Command::TYPE_CUSTOMER) {
        $mailSmarty->assign('CommandType', $commandType);
        // Si la preference est activee, on affiche en plus une colonne pour les qte d'UE
        if (Preferences::get('ProductCommandUEQty')) {
            $mailSmarty->assign('ProductCommandUEQty', 1);
        }
        $mailSmarty->assign('RemExcep', I18N::formatNumber($ProductCommand->getCustomerRemExcep()));
        if ($NewsDeliveryDest != "") {
            $Comment = $ProductCommand->getComment() . '<br>Livraison '
                    . $Dest->getName() . ' : ' . $NewsDeliveryDest;
        }
        else {
            $Comment = $ProductCommand->getComment();
        }
    }
    else {//SUPPLIER
        $Comment = $ProductCommand->getComment();
    }
    $ttcTotalPrice = $ProductCommand->getTotalPriceTTC();
    $mailSmarty->assign('CommandType', $commandType);
    $mailSmarty->assign('hasTVA', $hasTVA);
    $mailSmarty->assign('products', $productArray);
    $mailSmarty->assign('htTotalPrice',
            I18N::formatNumber($ProductCommand->getTotalPriceHT()));
    $mailSmarty->assign('ttcTotalPrice', I18N::formatNumber($ttcTotalPrice));
    $mailSmarty->assign('GlobalHanding', $ProductCommand->getHanding());
    $mailSmarty->assign('Port', $ProductCommand->getPort());
    $mailSmarty->assign('Packing', $ProductCommand->getPacking());
    $mailSmarty->assign('Insurance', $ProductCommand->getInsurance());
    $mailSmarty->assign('Comment', trim(stripslashes($Comment)));
    $mailSmarty->assign('Installment',
            I18N::formatNumber($ProductCommand->getInstallment()));
    if ($ttcTotalPrice < $ProductCommand->getInstallment()) {
        $ToPay = 0;
    }
    else {
        $ToPay = $ttcTotalPrice - $ProductCommand->getInstallment();
    }

    $mailSmarty->assign('ToPay', I18N::formatNumber($ToPay));

    if ($commandType == Command::TYPE_SUPPLIER) {
        $commandname = strtoupper(_('Supplier'));
        $mailSmarty->assign('commandtype', $commandname);
    }
    if ($commandType == Command::TYPE_CUSTOMER) {
        $commandname = strtoupper(_('Customer'));
        $mailSmarty->assign('commandtype', $commandname);
    }

    $date_souhaitee = I18N::formatDate($ProductCommand->getWishedStartDate());

    if ($ProductCommand->getWishedEndDate() != 0 &&
        $ProductCommand->getWishedEndDate() != 'NULL') {
        $date_souhaitee = sprintf(_('between %s and %s'), $date_souhaitee,
                I18N::formatDate($ProductCommand->getWishedEndDate()));
    }
    $mailSmarty->assign('dateSouhaitee', $date_souhaitee);

    $body = $mailSmarty->fetch('Product/ProductCommandMail.html');

    require_once('Objects/Alert.const.php');
    require_once('AlertSender.php');
    $uacCol = false;
    $params = array('NumCde'=> $ProductCommand->getCommandNo(), 'body'=>$body);
    ...
    if ($commandType == Command::TYPE_SUPPLIER) {
        CommandManager::$alertsToSend[] = array(
            'ALERT_SUPPLIER_COMMAND_RECEIPT',
            array(
                $params, $uacCol,
                array($destSite->getId(), $expSite->getId())
            )
        );
        //$res = AlertSender::send_ALERT_SUPPLIER_COMMAND_RECEIPT($params, $uacCol, array($destSite->getId(), $expSite->getId()));
    }
    if ($commandType == Command::TYPE_CUSTOMER) {
        CommandManager::$alertsToSend[] = array(
            'ALERT_CLIENT_COMMAND_RECEIPT',
            array(
                $params, $uacCol,
                array($destSite->getId(), $expSite->getId())
            )
        );
        //$res = AlertSender::send_ALERT_CLIENT_COMMAND_RECEIPT($params, $uacCol, array($destSite->getId(), $expSite->getId()));
    }
}
