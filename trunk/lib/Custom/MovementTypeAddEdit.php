 ...
 *
 */
class MovementTypeAddEdit extends GenericAddEdit {
    // MovementTypeAddEdit::onBeforeDisplay() {{{
    
    public function onBeforeDisplay() {
        if ($this->action == GenericController::FEATURE_EDIT) {
            $this->form->freeze('MovementType_ConstName');
        }

    }
    
    // }}}
    // MovementTypeAddEdit::onBeforeHandlePostData() {{{
    
    public function onBeforeHandlePostData(&$values) {
        if ($this->action == GenericController::FEATURE_ADD) {
            if (!preg_match('/^[a-zA-Z_]+$/', $values['MovementType_ConstName'])) {
                Template::errorDialog(_('You must provide a valid constant name'), $this->guessReturnURL());
                exit(1);
            }
            $values['MovementType_ConstName'] = strtoupper($values['MovementType_ConstName']);
        }
    }
